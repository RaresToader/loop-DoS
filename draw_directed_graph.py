...
def simplify_graph(graph, edges_attr):
    ...
    for cycle in cycles:
        ...
        if len(cycle) == 1:
            ...
            # TODO edited for our smaller Loops
            if edge_attr[1] <= 1:
                continue
            cycle.append(cycle[0])
            print_lines.append([cycle, edge_attr[1], edge_attr[1], [cycle[0], cycle[0]],
                                int(math.factorial(edge_attr[1]) / (2 * math.factorial(edge_attr[1] - 2)))])
            ...
            if edge_A_attr[1] <= edge_B_attr[1]:
                print_lines.append([cycle, len(affected_IPs), min(edge_A_attr[1], edge_B_attr[1]), [cycle[0], cycle[1]],
                                    edge_A_attr[1] * edge_B_attr[1] - len(
                                        set(edge_A_attr[0]).intersection(set(edge_B_attr[0]))) * len(
                                        set(edge_A_attr[0]).intersection(set(edge_B_attr[0])))])
            else:
                print_lines.append([cycle, len(affected_IPs), min(edge_A_attr[1], edge_B_attr[1]), [cycle[1], cycle[0]],
                                    edge_A_attr[1] * edge_B_attr[1] - len(
                                        set(edge_A_attr[0]).intersection(set(edge_B_attr[0]))) * len(
                                        set(edge_A_attr[0]).intersection(set(edge_B_attr[0])))])
            ...
            try:
                ...
                print_lines.append(
                    [cycle, len(affected_IPs), min(len(temp_edge_list_A), len(temp_edge_list_B)), min_edge,
                     len(temp_edge_list_A) * len(temp_edge_list_B) - len(
                         set(temp_edge_list_A).intersection(set(temp_edge_list_B))) * len(
                         set(temp_edge_list_A).intersection(set(temp_edge_list_B)))])
                all_ips_affected.update(affected_IPs)
