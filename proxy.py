...
if len(sys.argv) != 7:
    print(
        'python3 proxy.py <local_ip> <loop_probe_table_name> <sampled_payloads_file> <cycle_result_output> <start_port> <target_port>')
    exit(0)
...
class Proxy_core():
    def __init__(self, local_ip, cycle_ip_dict_file, scan_payload_file, scan_2nd_table_name, timeout, protocol_port,
                 start_port_number):
    ...
    def work_scheduler(self, cycle_ip_dict_file, scan_payload_file, scan_2nd_table_name, start_port_number):
        ...
        for cycle in cycle_ip_dict.keys():
            ...
            # TODO this was edited for our small case
            ...
            payload_to_initiate = payload_dict[initiate_probe_type][
                self.most_replied_probe_each_cluster[initiate_probe_type]]
            ...
            for i in range(0, SAMPLED_PAIRS_PER_CYCLE):
                if not host_A_ips[i] in self.ip_hostobj_mapping:
                    self.ip_hostobj_mapping[host_A_ips[i]] = Host(host_A_ips[i], self.protocol_port,
                                                                  ratelimit=RATE_LIMIT)
                if not host_B_ips[i] in self.ip_hostobj_mapping:
                    self.ip_hostobj_mapping[host_B_ips[i]] = Host(host_B_ips[i], self.protocol_port,
                                                                  ratelimit=RATE_LIMIT)
                loop_pair_object = Loop_pair(self.local_ip, start_port_number, self.ip_hostobj_mapping[host_A_ips[i]],
                                             self.ip_hostobj_mapping[host_B_ips[i]])
            ...
            if curr_time - start_time > self.timeout:
                ...
                for key in self.cycle_pair_mapping.keys():
                    ...
                    for pair in self.cycle_pair_mapping[key][0]:
                        line = str(pair.get_host_A_addr()) + ' : ' + str(pair.get_host_B_addr()) + ':' + str(
                            pair.total_rcv_counter) + '\n'
                        f.write(line)
                ...
                for key in self.cycle_pair_mapping.keys():
                    ...
                    line = str(key).strip() + ' : ' + str(temp_counter) + '/' + str(
                        len(self.cycle_pair_mapping[key][0])) + '\n'
                    lines.append(line)
...
proxy = Proxy_core(local_ip, cycle_ip_dict_file, scan_payload_file, scan_2nd_table_name, OVER_ALL_TIMEOUT, proto_port,
                   start_port)
